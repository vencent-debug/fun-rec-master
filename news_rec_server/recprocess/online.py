...
class OnlineServer(object):
    ...
    def _set_user_group(self):
        ...
        """
        self.user_group = {
            "1": ["金融","医疗卫生","教育培训","科技","能源矿产","交通运输"],
            "2": ["房产建筑","体育","制造业","军事国防","生态环境","旅游"],
            "3": ["住宿餐饮","农林牧渔","时政","社会民生","文化娱乐","信息产业"],
            "4": ["交通安全", "社会治安","灾害险情"]
        }
    ...
    def test(self):
        user_info = self.register_sql_sess.query(RegisterUser).filter(RegisterUser.userid == "123").first()
        print(user_info.age)

    def get_sort_news_by_data_rec_list(self, user_id,cate_id):
        # 通过轮询的方式获取用户的展示列表
        cate_dict_news_list = self._get_cate_news_list(user_id,cate_id)
        # 添加曝光内容
        # self._save_user_exposure(user_id, exposure_news_list)
        return cate_dict_news_list


    def _get_cate_news_list(self, user_id,cate_id, one_page_news_cnt=10):
        # 用户展示新闻列表
        user_news_list = []
        iter_cnt = 0
        # 给每个用户轮询每个类别的新闻，获取打散之后的新闻列表
        while len(user_news_list) != one_page_news_cnt:
            # cate_id_index = iter_cnt % len(cate_id_list)
            user_redis_key = "hot_list_news_cate:{}".format(cate_id)
            # cate_id = 'user_id_hot_list:'+news_cate_id
            cur_cate_cnt = 0
            # while self.reclist_redis_db.zcard(user_redis_key) > 0:
            # 摘取排名第一的新闻
            news_id_and_cate = self.reclist_redis_db.zrevrange(user_redis_key, iter_cnt, iter_cnt)[0]
            news_id = news_id_and_cate.split('_')[1] # 将新闻id切分出来
